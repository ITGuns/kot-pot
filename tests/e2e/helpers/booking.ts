...
export async function bookThrough(page: Page, opts: { date: string; party: number; time: string; first: string; last: string; email: string; phone: string; requests?: string; seating?: string }) {
  ...
  if (opts.seating) await page.getByLabel(/^Seating preference/).fill(opts.seating);
