...
export function Carousel({
  ...
  return (
    ...
    >
      <ul ref={track} tabIndex={0} aria-label={`${ariaLabel} slides`} className={cn("scrollbar-none flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth rounded-[22px] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-chili-400", trackClassName)}>
        {slides.map((s, i) => (
              ...
              {slides.map((_, i) => (
                <button
                  ...
                  className={cn(
                    ...
                    i === index ? "w-8 bg-chili-500" : dark ? "w-2 bg-ivory-50/30 hover:bg-ivory-50/60" : "w-2 bg-ink-900/20 hover:bg-ink-900/40",
                  )}
                />
              ))}
            </div>
