...
const STEPS = ["Date", "Party size", "Time", "Details", "Review"] as const;
const DRAFT_KEY = "kp-booking-draft";
...
export function BookingWizard({
  ...
  settings: Pick<BookingSettings, "minPartySize" | "maxPartySize" | "largePartyThreshold" | "turnTimeMinutes" | "seatingPreferences" | "occasions" | "bookingsEnabled" | "timezone">;
  ...
  if (!settings.bookingsEnabled) {
    return (
      <div className="rounded-[24px] border border-ink-900/10 bg-ivory-50 p-8 text-center shadow-card">
        <h2 className="font-display text-3xl text-ink-900">Online booking is paused</h2>
        <p className="mt-3 text-ink-700">Please call us at <a className="font-semibold text-ink-900" href={`tel:+1${restaurant.phone.replace(/\D/g, "")}`}>{restaurant.phone}</a> to reserve a table.</p>
      </div>
    );
  }

  if (result) return <Confirmation reservation={result} restaurant={restaurant} turnTime={settings.turnTimeMinutes} timezone={settings.timezone} />;
  ...
  return (
    <div ref={topRef} className="grid gap-8 lg:grid-cols-12 lg:gap-12">
      <div className="lg:col-span-8">
            ...
            return (
              <li key={label} className="flex flex-1 items-center gap-2">
                <button
                  ...
                  className={cn("flex items-center gap-2 text-[13px] font-semibold", current ? "text-ink-900" : done ? "text-ink-700" : "text-ink-500/60")}
                >
                  <span className={cn("flex h-7 w-7 items-center justify-center rounded-full font-label text-[13px] tracking-wide", current ? "bg-chili-600 text-ivory-50" : done ? "bg-ink-900 text-ivory-50" : "bg-ink-900/8 text-ink-500")}>
                    {done ? "✓" : i + 1}
                  </span>
                  <span className="hidden sm:inline">{label}</span>
                </button>
                {i < STEPS.length - 1 && <span className={cn("h-px flex-1", done ? "bg-ink-900" : "bg-ink-900/10")} />}
              </li>
            );
          })}
        </ol>
        ...
        {error && (
          <div role="alert" className="mt-6 rounded-2xl border border-chili-500/30 bg-chili-500/8 px-4 py-3 text-[14px] text-chili-700">
            {error}
          </div>
        )}
                ...
                <section aria-labelledby="step-date">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {quickDates(bookable.from)
                      ...
                      .map((q) => (
                        <button key={q.label} type="button" onClick={() => chooseDate(q.date)} className="h-9 rounded-full border border-ink-900/12 bg-ivory-50 px-4 text-[13px] font-semibold text-ink-800 transition hover:border-ink-900">
                  ...
                  <div className="mt-6 grid grid-cols-4 gap-3 sm:grid-cols-5 md:grid-cols-6">
                    {Array.from({ length: settings.maxPartySize - settings.minPartySize + 1 }, (_, i) => settings.minPartySize + i).map((n) => (
                      <button
                        ...
                        className={cn(
                          "flex aspect-square flex-col items-center justify-center rounded-[18px] border text-ink-900 transition-all",
                          party === n ? "border-chili-600 bg-chili-600 text-ivory-50 shadow-glow" : "border-ink-900/10 bg-ivory-50 hover:border-ink-900",
                        )}
                      >
                        <span className="font-display text-3xl leading-none">{n}</span>
                        <span className={cn("mt-1 text-[11px] uppercase tracking-[0.12em]", party === n ? "text-ivory-50/80" : "text-ink-500")}>{n === 1 ? "guest" : "guests"}</span>
                      </button>
                    ))}
                  </div>
                  <p className="mt-5 text-[13px] text-ink-500">
                    Parties of {settings.largePartyThreshold}+ are held as a request and confirmed by our team. More than {settings.maxPartySize} guests? Call{" "}
                    <a href={`tel:+1${restaurant.phone.replace(/\D/g, "")}`} className="font-semibold text-ink-900">{restaurant.phone}</a>.
                  </p>
                  <Button variant="ghost" size="sm" className="mt-6 text-ink-700" onClick={() => go(0)}>← Change date</Button>
                </section>
              )}
                ...
                <section aria-labelledby="step-time">
                  ...
                  <div className="mt-6" aria-live="polite">
                    ...
                    {!loadingSlots && slots && !slots.ok && (
                      <div className="rounded-[20px] border border-ink-900/10 bg-ivory-50 p-6">
                        <p className="font-semibold text-ink-900">{slots.message}</p>
                  ...
                  <div className="mt-6 flex gap-2">
                    <Button variant="ghost" size="sm" className="text-ink-700" onClick={() => go(1)}>← Change guests</Button>
                    <Button variant="ghost" size="sm" className="text-ink-700" onClick={() => go(0)}>Change date</Button>
                  </div>
                </section>
              )}
                ...
                <section aria-labelledby="step-details">
                  ...
                  >
                    ...
                    {settings.seatingPreferences.length > 0 ? (
                      ...
                      <TextField label="Seating preference" name="seatingPreference" optional maxLength={60} placeholder="e.g. near the window" value={details.seatingPreference} onChange={(v) => setDetails({ ...details, seatingPreference: v })} />
                    )}
                    ...
                    <label className="sm:col-span-2">
                      <span className="mb-1.5 block text-[13px] font-semibold text-ink-800">Special requests <span className="font-normal text-ink-500">(optional)</span></span>
                      <textarea
                        ...
                        className="w-full rounded-2xl border border-ink-900/12 bg-ivory-50 px-4 py-3 text-[15px] text-ink-900 placeholder:text-ink-500/60 focus:border-chili-400 focus:outline-none"
                      />
                    </label>
                    <div className="flex flex-wrap items-center gap-3 sm:col-span-2">
                      <Button type="submit" size="lg" arrow>Review reservation</Button>
                      <Button type="button" variant="ghost" className="text-ink-700" onClick={() => go(2)}>← Change time</Button>
                    </div>
                  </form>
                </section>
              )}
                ...
                <section aria-labelledby="step-confirm">
                  ...
                  <div className="mt-6 rounded-[24px] border border-ink-900/10 bg-ivory-50 p-6 shadow-card sm:p-8">
                    <dl className="grid gap-5 sm:grid-cols-3">
                      ...
                      <div><dt className="eyebrow text-ink-500">Time</dt><dd className="mt-1 font-display text-xl text-ink-900">{time12(time!)}</dd></div>
                      <div><dt className="eyebrow text-ink-500">Party</dt><dd className="mt-1 font-display text-xl text-ink-900">{party} {party === 1 ? "guest" : "guests"}</dd></div>
                    </dl>
                    <div className="mt-5 border-t border-ink-900/10 pt-5 text-[15px] text-ink-700">
                      <p className="font-semibold text-ink-900">{details.firstName} {details.lastName}</p>
                  ...
                  {party! >= settings.largePartyThreshold && (
                    <p className="mt-4 rounded-2xl bg-bronze-400/20 px-4 py-3 text-[14px] text-wood-700">Parties of {settings.largePartyThreshold}+ are submitted as a request. We&apos;ll confirm by phone or email.</p>
                  )}
                  <div className="mt-6 flex flex-wrap items-center gap-3">
                    <Button size="lg" onClick={submit} disabled={submitting} arrow={!submitting}>
                      {submitting ? "Reserving…" : "Confirm reservation"}
                    </Button>
                    <Button variant="ghost" className="text-ink-700" onClick={() => go(3)} disabled={submitting}>← Edit details</Button>
                  </div>
                </section>
              )}
            </motion.div>
      ...
      <aside className="hidden lg:col-span-4 lg:block">
        <div className="sticky top-28 rounded-[24px] bg-ink-900 p-6 text-ivory-50 shadow-lift">
          <p className="eyebrow text-chili-300">Your reservation</p>
          ...
          <div className="mt-6 border-t border-ivory-50/10 pt-5 text-[13px] leading-relaxed text-ivory-100/70">
            <p className="font-semibold text-ivory-50">{restaurant.name}</p>
            <p>{restaurant.addressLine1}, {restaurant.city}, {restaurant.state} {restaurant.zip}</p>
            <p className="mt-2">Questions? Call {restaurant.phone}.</p>
          </div>
        </div>
      </aside>
    </div>
...
function SlotGrid({ slots, label, note, value, onChoose, onOtherDay }: { slots: SlotInfo[]; label: string | null; note: string | null; value: string | null; onChoose: (t: string) => void; onOtherDay: () => void }) {
  ...
  return (
    <div className="space-y-6">
      {note && <p className="rounded-2xl bg-bronze-400/20 px-4 py-2.5 text-[14px] text-wood-700">{note}</p>}
      {!anyAvailable && (
        <div className="rounded-[20px] border border-ink-900/10 bg-ivory-50 p-6">
          <p className="font-semibold text-ink-900">No times left for this party size on this day.</p>
          <Button size="sm" variant="secondary" className="mt-4" onClick={onOtherDay}>Try another day</Button>
        </div>
      )}
      {groups.map((g) => (
        <div key={g.name}>
          <h3 className="eyebrow text-ink-500">{g.name}</h3>
            ...
            {g.slots.map((s) => (
              <button
                ...
                className={cn(
                  ...
                  value === s.time
                    ? "border-chili-600 bg-chili-600 text-ivory-50 shadow-glow"
                    : s.available
                      ? "border-ink-900/12 bg-ivory-50 text-ink-900 hover:border-ink-900"
                      : "cursor-not-allowed border-ink-900/8 bg-ink-900/4 text-ink-900/30 line-through",
                )}
              >
                {s.label}
              </button>
...
function SummaryRow({ label, value, onEdit }: { label: string; value: string; onEdit?: () => void }) {
  return (
    <div className="flex items-baseline justify-between gap-4 border-b border-ivory-50/10 pb-3">
      <dt className="text-[13px] text-ivory-100/70">{label}</dt>
      <dd className="flex items-baseline gap-3 text-right text-[15px] font-semibold">
        {value}
        {onEdit && (
          <button type="button" onClick={onEdit} className="text-[12px] font-semibold text-chili-300 hover:underline">
            Edit
          </button>
        )}
      </dd>
...
function TextField({ label, error, optional, onChange, ...rest }: { label: string; error?: string; optional?: boolean; onChange: (v: string) => void } & Omit<React.InputHTMLAttributes<HTMLInputElement>, "onChange">) {
  ...
  return (
    <label htmlFor={id} className="block">
      ...
      <input
        ...
        className={cn(
          "h-12 w-full rounded-full border bg-ivory-50 px-4 text-[15px] text-ink-900 placeholder:text-ink-500/60 focus:outline-none",
          error ? "border-chili-500 focus:border-chili-500" : "border-ink-900/12 focus:border-chili-400",
        )}
      />
      {error && <span id={`${id}-err`} className="mt-1 block text-[12px] font-medium text-chili-600">{error}</span>}
    </label>
  );
}
...
function SelectField({ label, name, value, options, onChange }: { label: string; name: string; value: string; options: string[]; onChange: (v: string) => void }) {
  ...
  return (
    <label htmlFor={id} className="block">
      <span className="mb-1.5 block text-[13px] font-semibold text-ink-800">{label} <span className="font-normal text-ink-500">(optional)</span></span>
      <select id={id} name={name} value={value} onChange={(e) => onChange(e.target.value)} className="h-12 w-full appearance-none rounded-full border border-ink-900/12 bg-ivory-50 px-4 text-[15px] text-ink-900 focus:border-chili-400 focus:outline-none">
