...
export function DatePicker({
  ...
  return (
    <div className="rounded-[22px] border border-ink-900/10 bg-ivory-50 p-4 sm:p-6">
      <div className="flex items-center justify-between">
        <button type="button" onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1, 12))} disabled={!canPrev} aria-label="Previous month" className="flex h-10 w-10 items-center justify-center rounded-full border border-ink-900/10 text-ink-900 transition hover:bg-ink-900/5 disabled:opacity-30">
        ...
        <h3 className="font-display text-xl text-ink-900" aria-live="polite">{monthLabel}</h3>
        <button type="button" onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1, 12))} disabled={!canNext} aria-label="Next month" className="flex h-10 w-10 items-center justify-center rounded-full border border-ink-900/10 text-ink-900 transition hover:bg-ink-900/5 disabled:opacity-30">
      ...
      <div className="mt-4 grid grid-cols-7 gap-1 text-center">
        {DAY_SHORT.map((d) => (
          <span key={d} className="py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-ink-500">{d}</span>
        ))}
        {cells.map((d, i) => {
          ...
          return (
            <button
              ...
              className={cn(
                ...
                selected ? "bg-chili-600 text-ivory-50 shadow-glow" : disabled ? "text-ink-900/25 line-through decoration-ink-900/20" : "text-ink-900 hover:bg-ink-900/8",
                isToday && !selected && "ring-1 ring-inset ring-chili-500/60",
              )}
            >
              {Number(d.slice(-2))}
            </button>
      ...
      <p className="mt-4 text-[12px] text-ink-500">Crossed-out dates are closed. Bookable through {parseDate(to).toLocaleDateString("en-US", { month: "long", day: "numeric" })}.</p>
    </div>
  );
}
...
export function quickDates(from: string): { label: string; date: string }[] {
  ...
  out.push({ label: "Sunday", date: addDays(from, toSun) });
  return out;
}
